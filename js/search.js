class DictionarySearch {
  constructor() {
    this.apiUrl = "https://potipress.com/api/definitions";
    this.init();
  }
  ...
  init() {
    document
      ...
      .addEventListener("submit", (event) => this.handleSearch(event));
  }
  ...
  async handleSearch(event) {
    ...
    try {
      ...
      if (result.definition) {
        const message = this.formatMessage(strings.response.success, {
          message: `Word: ${result.word}, Definition: ${result.definition}`,
        });
        document.getElementById("result").innerText = message;
      } else {
        const message = this.formatMessage(strings.response.error, {
